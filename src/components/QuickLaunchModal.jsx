      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
        <div className="w-full max-w-lg rounded-xl border border-emerald-500/20 bg-[#080c15]/98 p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2 mb-3">
            ...
            {result.engine && (
              <span className={`text-[8px] font-mono px-1.5 py-0.5 rounded ml-auto ${result.engine === "claude" ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-yellow-500/10 text-yellow-400 border border-yellow-500/20"}`}>
                {result.engine === "claude" ? "CLAUDE" : "SIMULATION"}
              </span>
            )}
          </div>
          {result.result && (
